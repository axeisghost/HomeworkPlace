...
int main(int argc, char *argv[]) {
	...
	char datafile_name[strlen(name_without_ext) + 2];
	sprintf(datafile_name, "%s.c", name_without_ext);
	FILE* outfile2 = fopen(datafile_name, "w");
	fprintf(outfile2, "const unsigned short %s_data[%d] = {", name_without_ext, height * width);
	int kk;
	int jj;
	unsigned char bmp_pixel_chrome;
	unsigned short gba_pixel;
	for (kk = (0x36 + width * (height - 1) * 4); kk >= 0x36; kk -= (width * 4)) {
		for (jj = kk; jj < (kk + width * 4); jj += 4) {
			gba_pixel = 0;
			bmp_pixel_chrome = (unsigned char)data_arr[jj] / 8;
			gba_pixel += (bmp_pixel_chrome << 10);
			bmp_pixel_chrome = (unsigned char)data_arr[jj + 1] / 8;
			gba_pixel += (bmp_pixel_chrome << 5);
			bmp_pixel_chrome = (unsigned char)data_arr[jj + 2] / 8;
			gba_pixel += bmp_pixel_chrome;
			fprintf(outfile2, "0x%x, ", gba_pixel);
		}
	}
	fprintf(outfile2, "};");
	fclose(outfile2);
